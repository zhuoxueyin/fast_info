<template>
  <router-link
    :to="`/items/${item.id}`"
    class="block bg-white rounded-lg border border-slate-200 hover:border-emerald-300 hover:shadow-md transition p-4 cursor-pointer"
  >
    ...
    <h3 :class="compact ? 'text-sm' : 'text-base'" class="font-semibold text-slate-900 leading-snug mb-2 line-clamp-2">
      {{ item.title }}
    </h3>
    <p v-if="!compact && item.summary" class="text-sm text-slate-600 leading-relaxed line-clamp-2 mb-3">
      {{ item.summary }}
    </p>
