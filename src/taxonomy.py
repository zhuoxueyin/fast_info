...
_TEXT_L1_KEYWORDS = {
    ...
    "体育": [
        "足球", "篮球", "nba", "cba", "世界杯", "欧冠", "奥运会", "f1", "马拉松",
        "网球", "乒乓球", "羽毛球",
    ],
    # 娱乐:娱乐圈新闻,聚焦明星八卦/影视/音乐/演唱会
    "娱乐": [
        "电影", "电视剧", "票房", "导演", "演员", "奥斯卡", "金球奖",
        "音乐", "歌手", "专辑", "演唱会", "billboard",
        "明星", "偶像", "选秀", "八卦", "娱乐圈", "真人秀", "综艺节目",
    ],
    # 动漫:ACG 文化(动画/漫画/二次元/声优)
    "动漫": [
        "动漫", "动画", "动画片", "动画电影", "漫画", "二次元", "acg",
        "cosplay", "声优", "配音", "番剧", "新番",
        "巨人", "海贼王", "火影", "柯南", "鬼灭", "咒术", "进击的巨人",
    ],
    # 游戏:所有游戏相关(PC/主机/手游/电竞/产业)
    "游戏": [
        "游戏", "电子游戏", "pc游戏", "端游", "单机", "独立游戏",
        "主机游戏", "ps5", "ps4", "switch", "xbox", "任天堂", "暴雪",
        "手游", "手机游戏", "原神", "米哈游", "steam",
        "电竞", "电竞赛事", "英雄联盟", "lol", "dota", "dota2", "王者荣耀",
    ],
    "财经": [
...
def _l1_from_text(text: Optional[str]) -> str:
    ...
    if scores["汽车"] > 0 and scores["财经"] > 0:
        return "汽车"
    # 游戏/动漫容易混淆:游戏优先(如「原神是二次元风格」这种,显然归游戏更准)
    if scores["游戏"] > 0 and scores["动漫"] > 0:
        return "游戏"
...
def suggest_l2(l1: str, content: str = "") -> str:
    ...
    rules = {
        ...
        "娱乐": [("明星", ["明星", "偶像", "八卦", "选秀", "娱乐圈", "演唱会"]),
                 ("影视", ["电影", "电视剧", "票房", "导演", "演员", "奥斯卡", "金球奖", "金马奖"]),
                 ("音乐", ["音乐", "歌手", "专辑", "演唱会", "billboard"]),
                 ("综艺", ["综艺", "真人秀", "选秀节目"])],
        "动漫": [("动画", ["动画", "动画片", "番剧", "新番", "动画电影"]),
                 ("漫画", ["漫画", "连载", "单行本"]),
                 ("二次元", ["二次元", "cosplay", "acg"]),
                 ("声优", ["声优", "配音", "cv"])],
        "游戏": [("PC游戏", ["pc游戏", "端游", "steam", "单机", "独立游戏"]),
                 ("主机游戏", ["ps5", "ps4", "switch", "xbox", "任天堂", "主机"]),
                 ("手游", ["手游", "手机游戏", "原神", "米哈游"]),
                 ("电竞", ["电竞", "英雄联盟", "lol", "dota", "dota2", "csgo", "王者荣耀", "电竞赛事", "总决赛", "msi", "s赛"]),
                 ("游戏产业", ["游戏产业", "暴雪", "腾讯游戏", "网易游戏"])],
